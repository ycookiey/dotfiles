...
if (!$currentBash -and (tp $GitBash)) {
    ...
    wh "  CLAUDE_CODE_GIT_BASH_PATH -> git-bash" -Fg Cyan
}

# ─── Step 2: Windows 機能有効化 ───
wh "[WSL 1/4] Windows 機能チェック..." -Fg Cyan
...
if ($wslF.RestartNeeded -or $vmpF.RestartNeeded) {
    ...
    wh "  機能を有効化しました。再起動後に setup.ps1 を再実行してください。" -Fg Yellow
    return
}
...
if ($wslStatus -match "REGDB_E_CLASSNOTREG|エラー" -or $LASTEXITCODE -ne 0) {
    ...
    wh "  インストール完了" -Fg Green
} else {
    wh "  正常" -Fg Green
}

# ─── Step 4: Ubuntu ───
wh "[WSL 3/4] Ubuntu 確認..." -Fg Cyan
$ubuntuPkg = Get-AppxPackage *Ubuntu* | Select-Object -First 1

if (-not $ubuntuPkg) {
    wh "  Ubuntu をダウンロード中... (数分かかります)" -Fg Yellow
    ...
    wh "  インストール済み" -Fg Green
}

# ─── Step 5: Ubuntu 初期化 ───
wh "[WSL 4/4] Ubuntu 初期化..." -Fg Cyan
wsl --set-default-version 2 2>$null | Out-Null

$exe = Get-ChildItem -Path $ubuntuPkg.InstallLocation -Filter "ubuntu*.exe" -ea 0 |
    Select-Object -First 1 -ExpandProperty FullName

if ($exe) {
    $proc = Start-Process -FilePath $exe -ArgumentList "install --root" -Wait -NoNewWindow -PassThru
    if ($proc.ExitCode -notin @(0, -1)) {
        wh "  初期化失敗 (Code: $($proc.ExitCode))" -Fg Red
        wh "  0x80370114 の場合、BIOS で CPU 仮想化 (VT-x/AMD-V) を有効にしてください" -Fg Yellow
        return
    }
}
...
if ($test -match "ok") {
    ...
    wh "  CLAUDE_CODE_GIT_BASH_PATH -> WSL bash に切り替えました" -Fg Green
} else {
    wh "  WSL 疎通失敗。git-bash を維持します。" -Fg Yellow
}
