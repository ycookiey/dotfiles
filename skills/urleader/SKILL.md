...
上流member待ちが発生し、かつ下流write-taskのsetupが高コスト(`worktree-init.sh`あり等)なら、待ち時間にstep 1を`run_in_background`で先行実行してよい。prepはprompt非依存(`--task-id`のみ依存)。完了通知で`WORKTREE_ROOT`を回収し、上流出力が揃い次第spawnする。詳細・トリガー条件は agent-team.md「先行prep」参照。
