...
```bash
python ~/.claude/skills/search-sessions/history.py list [--project NAME] [--days N] [--include-current]
```
...
| `--days` | 30 | 直近N日 |
| `--include-current` | false | 現在のセッションを含める（デフォルトで除外） |
...
```bash
python ~/.claude/skills/search-sessions/history.py search QUERY [--project NAME] [--days N] [--limit N] [--regex] [--include-current]
```
...
- `list`/`search` は現在実行中のセッションを自動除外する（キーワードが毎回自己ヒットするのを防ぐため）。現在のセッションは `os.getcwd()` から派生したプロジェクトディレクトリ内で最新mtimeの `.jsonl` を検出する。含めたい場合は `--include-current` を指定
