...
### Phase 3.5: uv tools — install/uv-tools.ps1

uv が利用可能な場合のみ実行。install/uv-tools.json に列挙した CLI ツールを `uv tool install` で導入。
basedpyright・ruff（Neovim の Python LSP）、aider-chat 等。失敗しても続行。
...
| uv tool CLI（basedpyright, ruff 等） | install/uv-tools.json |
